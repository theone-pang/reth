...
use reth_eth_wire::{PbftConsensusMessage, PbftConsensusMessageHeader};
...
pub enum ConsensusEvent {
    ...
    PeerMessage(PeerId, PbftConsensusMessage),
    BlockValid(B256),
    BlockInvalid(B256),
...
) -> Result<PbftConsensusMessage, PbftError> {
    PbftConsensusMessage::decode(&mut bytes.to_vec().as_slice()).map_err(|err| {
...
fn parse_consensus_message_header(
    bytes: &reth_primitives::Bytes,
) -> Result<PbftConsensusMessageHeader, PbftError> {
    PbftConsensusMessageHeader::decode(&mut bytes.to_vec().as_slice()).map_err(|err| {
        PbftError::SerializationError(
    ...
    Client: BlockReaderIdExt + StageCheckpointReader + 'static,
{
    match incoming_event {
        ...
        ConsensusEvent::BlockCommit((block_id, committing)) => {
            consensus.on_block_commit(block_id, committing, state)?
        }
