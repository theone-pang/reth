...
use reth_eth_wire::{
    ClayerBlock, PbftConsensusMessage, PbftConsensusMessageHeader, PbftMessage, PbftMessageInfo,
    PbftMessageType, PbftNewValidator, PbftNewView, PbftSeal, PbftSignedVote,
};
...
impl ParsedMessage {
    ...
    pub fn parse_header(
        header_bytes: &Bytes,
    ) -> std::result::Result<PbftConsensusMessageHeader, PbftError> {
        let header: PbftConsensusMessageHeader =
            match PbftConsensusMessageHeader::decode(&mut header_bytes.to_vec().as_slice()) {
                Ok(header) => header,
                Err(err) => {
                    return Err(PbftError::SerializationError(
    ...
    pub fn get_block_hash(&self) -> Option<B256> {
        match &self.message {
            PbftMessageWrapper::Message(m) => Some(m.block_id),
            PbftMessageWrapper::NewView(m) => {
                //panic!("ParsedPeerMessage.get_block_id found a new view message!");
                Some(m.block_id)
            }
            PbftMessageWrapper::Seal(_) => {
