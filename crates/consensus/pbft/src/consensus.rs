...
use reth_rpc_types::{engine::PayloadId, BlockNumberOrTag, ExecutionPayloadV1, ExecutionPayloadV2};
...
use reth_eth_wire::{
    ClayerBlock, ClayerConsensusEvent, ClayerConsensusMessageAgentTrait, ClayerExecutionPayload,
    ClayerSignature, PbftConsensusMessage, PbftConsensusMessageHeader, PbftMessage,
...
use crate::{
    ...
    incoming_msg_queue::IncomingMsgQueue,
    timing::{retry_until_ok, Timeout},
    ClayerConsensusMessageAgent,
};
    ...
    Client: BlockReaderIdExt + StageCheckpointReader + 'static,
{
    ...
    pub fn initialize(&mut self, block: ClayerBlock, config: &PbftConfig, state: &mut PbftState) {
        ...
        if block.block_num() > 0 {
            ...
            if state.is_validator() {
                let connected_peers: Vec<PeerId> = self.agent.peers();
        ...
        self.service.confirm_block(None).unwrap_or_else(|err| {
            error!(target: "consensus::cl","Couldn't confirm block on startup due to error: {}", err)
        });

        // Primary initializes a block
        if state.is_primary() {
            match self.service.initialize_block(state.seq_num, state.validators.accounts()) {
                Ok(_) => {}
                Err(err) => {
                    error!(target: "consensus::cl","Couldn't initialize block on startup,{}", err);
                }
            }
        }
        state.block_publishing_timeout.start();
    }
    ...
    pub fn on_peer_message(
        ...
        if !state.is_validator() {
            let seq_num = msg.info().seq_num;
            match self.client.latest_header() {
                Ok(v) => {
                    if let Some(latest_header) = v {
                        //info!(target: "consensus::cl","not validator, some msg:{:?},client:{:?}", seq_num,latest_header.header.number);
                        if latest_header.header.number == 0 && seq_num > 1 {
                            match msg_type {
                                PbftMessageType::AnnounceBlock
                                | PbftMessageType::ViewChange
                                | PbftMessageType::NewView => {
                                    //info!(target: "consensus::cl","not validator, some:msg:{:?},client:{:?}", seq_num,latest_header.header.number);
                                    warn!(target: "consensus::cl","validator node is syncing, number:{:?}", latest_header.header.number);
                                    if let Some(blockhash) = msg.get_block_hash() {
                                        if !blockhash.is_zero() {
                        ...
                        //else if latest_header.header.number < seq_num {
                        // info!(target: "consensus::cl","not validator, some:msg:{:?},client:{:?}", seq_num,latest_header.header.number);
                        //return Ok(());
                        //}
                    } else {
                        warn!(target: "consensus::cl"," get latest header None:{}", seq_num);
                    }
                }
                Err(e) => {
                    warn!(target: "consensus::cl"," get latest header error:{}", seq_num);
                }
            }
        ...
        match self.client.latest_header() {
            Ok(v) => {
                if let Some(latest_header) = v {
                    //info!(target: "consensus::cl","validator, some:msg:{:?},client:{:?}", seq_num,latest_header.header.number);
                    if latest_header.header.number == 0 && seq_num > 1 {
                        match msg_type {
                            PbftMessageType::AnnounceBlock | PbftMessageType::ViewChange => {
                                self.msg_log.add_message(msg.clone());
                                //info!(target: "consensus::cl","validator, some:msg:{:?},client:{:?}", seq_num,latest_header.header.number);
                                if let Some(blockhash) = msg.get_block_hash() {
                                    if !blockhash.is_zero() {
                                        if !self.announce_block.contains_key(&blockhash) {
                                            info!(target: "consensus::cl","validator node start syncing, number:{:?},hash:{:?}", latest_header.header.number, blockhash);
                                            self.announce_block
                                                .insert(blockhash, msg.info().seq_num);
                                            let _ = self.service_mut().announce_block(blockhash);
                                        }
                                    }
                                };
                            ...
                            PbftMessageType::NewView => {
                                match self.client.get_stage_checkpoint(StageId::Finish) {
                                    Ok(v) => {
                                        if let Some(stagecheckpoint) = v {
                                            //info!(target: "consensus::cl","validator, some:client:{:?}", stagecheckpoint.block_number);
                                            if stagecheckpoint.block_number == (seq_num - 1) {
                                                let blockhash = msg.get_block_hash().unwrap();
                                                info!(target: "consensus::cl","validator node end syncing, number:{:?},hash:{:?}", latest_header.header.number, blockhash);
                                                let _ =
                                                    self.service_mut().announce_block(blockhash);
                                            }
                                        }
                                    }
                                    Err(e) => {
                                        warn!(target: "consensus::cl"," get stage checkpoint error:{:?}", seq_num);
                                    }
                                };
                                return Ok(());
                            }
    ...
    fn handle_commit(
        ...
        if info.seq_num == state.seq_num && state.phase == PbftPhase::Committing {
            ...
            if has_matching_pre_prepare && has_required_commits {
                let _ = self.service.commit_block(block_id.clone()).map_err(|err| {
                    PbftError::ServiceError(
                        format!("Failed to commit block {:?}", hex::encode(&block_id)),
                        err.to_string(),
                    )
                })?;
                self.consensus_event_queue
                    .push_front_msg(ClayerConsensusEvent::BlockCommit((block_id, true)));
    ...
    fn handle_view_change(
        ...
        {
            let block_hash = match self.client.latest_header() {
                Ok(v) => {
                    if let Some(latest_header) = v {
                        latest_header.hash
                    } else {
                        warn!(target: "consensus::cl"," get block hash None");
                        B256::ZERO
                    }
                }
                Err(e) => {
                    warn!(target: "consensus::cl","get block hash None error");
                    B256::ZERO
                }
            };
    ...
    fn handle_new_view(
        ...
        self.service.confirm_block(None).map_err(|err| {
            PbftError::ServiceError(
                "Couldn't confirm block after view change".into(),
                err.to_string(),
            )
        })?;
        if state.is_primary() {
            self.service.initialize_block(state.seq_num, state.validators.accounts()).map_err(
                |err| {
                    PbftError::ServiceError(
    ...
    pub fn sync_seal(&mut self, state: &mut PbftState) -> Result<(), PbftError> {
        ...
        if state.block_publishing_timeout.duration().as_secs() > interval {
            return Ok(());
        }
    ...
    pub fn on_block_new(
        ...
        self.service
            .check_blocks(
                PayloadId::new(block.payload_id.to_vec().try_into().unwrap_or_else(
    ...
    fn catchup(
        ...
        self.consensus_event_queue
            .push_front_msg(ClayerConsensusEvent::BlockCommit((seal.block_id.clone(), false)));
    ...
    pub fn on_block_commit(
        ...
        self.service.confirm_block(Some(block_id)).map_err(|err| {
            error!(target: "consensus::cl","Couldn't confirm block after commit");
            PbftError::ServiceError(
                "Couldn't confirm block after commit".to_string(),
                err.to_string(),
            )
        })?;
        ...
        if !is_catching_up {
            if state.is_primary() {
                self.service.initialize_block(state.seq_num, state.validators.accounts()).map_err(
                    |err| {
                        error!(target: "consensus::cl","Couldn't confirm initialize after commit");
                        PbftError::ServiceError(
                            "Couldn't confirm initialize after commit".to_string(),
                            err.to_string(),
                        )
                    },
                )?;
    ...
    fn broadcast_bootstrap_commit(
        ...
        //create header
        let header = PbftConsensusMessageHeader {
        ...
        let clayer_msg = PbftConsensusMessage {
            header_bytes,
        ...
        F: Fn(&PbftMessage) -> Result<(), PbftError>,
    {
        ...
        let header: PbftConsensusMessageHeader =
            PbftConsensusMessageHeader::decode(&mut vote.header_bytes.to_vec().as_slice())
                .map_err(|err| {
                    PbftError::SerializationError(
    ...
    pub fn try_publish(&mut self, state: &mut PbftState) -> Result<(), PbftError> {
        ...
        if !state.block_publishing_timeout.check_expired() {
            return Ok(());
        }
        state.block_publishing_timeout.start();
        ...
        info!(target: "consensus::cl","===================================try_publish {}============================================",state.seq_num);
    ...
    fn broadcast_block_new(
        ...
        let msg: ClayerBlock =
            ClayerBlock { info, block: payload, seal_bytes, payload_id: payload_id.0 };
    ...
    pub fn start_view_change(&mut self, state: &mut PbftState, view: u64) -> Result<(), PbftError> {
        ...
        let num = if state.seq_num > 3 { state.seq_num - 2 } else { state.seq_num - 1 };

        let block_hash =
            match self.client.sealed_header_by_number_or_tag(BlockNumberOrTag::Number(num)) {
                Ok(v) => {
                    if let Some(latest_header) = v {
                        latest_header.hash
                    } else {
    ...
    pub fn get_block_id(&mut self) -> Result<u64, PbftError> {
        match self.service.get_block_id() {
            Ok(n) => Ok(n),
            Err(err) => {
                Err(PbftError::ServiceError("Couldn't get block number".into(), err.to_string()))
            }
        }
    }
}
