...
use reth_network_api::NetworkSecretKey;
use reth_node_api::{
...
use reth_pbft_consensus::{AuthHttpConfig, ClayerConsensusMessageAgentRef, PbftConsensusBuilder};
use reth_primitives::format_ether;
use reth_provider::providers::{BlockchainProvider, ConsensusProvider};
    ...
    >,
{
    ...
    async fn launch_node(
        ...
        let (pipeline, client) = if ctx.is_dev() {
            ...
            //===============================================================================
            let consensus_db = ConsensusProvider::new(ctx.provider_factory().clone())?;
            let consensus_agent = ClayerConsensusMessageAgentRef.clone();
            ...
            let jwt_path = ctx
                ...
                .auth_jwtsecret
                .clone()
                .ok_or(eyre::eyre!("jwt file no config"))?;
            let jwt_secret_bytes = jwt_secret_bytes(jwt_path)?;
            let secret_key = ctx.components().network().network_secret_key();
            let auth_config =
                AuthHttpConfig { port: ctx.node_config().rpc.auth_port, auth: jwt_secret_bytes };
            let mut task = PbftConsensusBuilder::new(
                secret_key.clone(),
                Arc::clone(&ctx.node_config().chain),
                ctx.blockchain_db().clone(),
                consensus_agent,
                consensus_db,
                auth_config,
...
fn jwt_secret_bytes(default_jwt_path: std::path::PathBuf) -> eyre::Result<Vec<u8>> {
    let jwt_secret_len: usize = 64;
    let hex: String = match std::fs::read_to_string(default_jwt_path) {
        Ok(hex) => hex,
        Err(_) => return Err(eyre::eyre!("jwt read failed")),
    };
    let s: &str = hex.as_ref();
    let hex: &str = s.trim().trim_start_matches("0x");
    if hex.len() != jwt_secret_len {
        Err(eyre::eyre!("invalid jwt secret length"))
    } else {
        let hex_bytes = alloy_primitives::hex::decode(hex)?;
        Ok(hex_bytes)
    }
}
