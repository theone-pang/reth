...
use reth_db_api::{
    models::{
        ...
        AccountBeforeTx, ClientVersion, CompactU256, ConsensusBytes, ShardedKey,
...
tables! {
    ...
    table ConsensusNumber<Key = BlockHash, Value = BlockNumber>;

    /// Stores the consensus content corresponding to a block hash.
    table ConsensusContent<Key = BlockHash, Value = ConsensusBytes>;
}
