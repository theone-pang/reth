...
use crate::{ClayerConsensusMsg, EthVersion, SharedTransactions};
...
pub enum EthMessage {
    ...
    /// Represents a Consensus message broadcast to the network.
...
impl EthMessage {
    ...
    pub const fn message_id(&self) -> EthMessageID {
        match self {
            ...
            Self::Consensus(_) => EthMessageID::Consensus,
        }
    }
}
...
impl Encodable for EthMessage {
    fn encode(&self, out: &mut dyn BufMut) {
        match self {
            ...
            Self::Consensus(msg) => msg.encode(out),
        }
    }
    fn length(&self) -> usize {
        match self {
...
pub enum EthBroadcastMessage {
    ...
    /// Represents a consensus broadcast message.
    Consensus(ClayerConsensusMsg),
}
